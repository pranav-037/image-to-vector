var __getOwnPropNames = Object.getOwnPropertyNames;
var __commonJS = (cb, mod) => function __require() {
  return mod || (0, cb[__getOwnPropNames(cb)[0]])((mod = { exports: {} }).exports, mod), mod.exports;
};

// browser-external:path
var require_path = __commonJS({
  "browser-external:path"(exports, module2) {
    module2.exports = Object.create(new Proxy({}, {
      get(_2, key) {
        if (key !== "__esModule" && key !== "__proto__" && key !== "constructor" && key !== "splice") {
          console.warn(`Module "path" has been externalized for browser compatibility. Cannot access "path.${key}" in client code. See https://vite.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.`);
        }
      }
    }));
  }
});

// browser-external:fs
var require_fs = __commonJS({
  "browser-external:fs"(exports, module2) {
    module2.exports = Object.create(new Proxy({}, {
      get(_2, key) {
        if (key !== "__esModule" && key !== "__proto__" && key !== "constructor" && key !== "splice") {
          console.warn(`Module "fs" has been externalized for browser compatibility. Cannot access "fs.${key}" in client code. See https://vite.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.`);
        }
      }
    }));
  }
});

// node_modules/esm-potrace-wasm/dist/index.js
var A;
var I;
var g;
var C;
var Q;
var B;
var E = void 0 !== E ? E : {};
var i = Object.assign({}, E);
var D = [];
var o = (A3, I2) => {
  throw I2;
};
var y = "object" == typeof window;
var h = "function" == typeof importScripts;
var G = "object" == typeof process && "object" == typeof process.versions && "string" == typeof process.versions.node;
var a = "";
G ? (a = h ? require_path().dirname(a) + "/" : __dirname + "/", B = () => {
  Q || (C = require_fs(), Q = require_path());
}, A = function(A3, I2) {
  var g2 = _(A3);
  return g2 ? I2 ? g2 : g2.toString() : (B(), A3 = Q.normalize(A3), C.readFileSync(A3, I2 ? void 0 : "utf8"));
}, g = (I2) => {
  var g2 = A(I2, true);
  return g2.buffer || (g2 = new Uint8Array(g2)), g2;
}, I = (A3, I2, g2) => {
  var E2 = _(A3);
  E2 && I2(E2), B(), A3 = Q.normalize(A3), C.readFile(A3, function(A4, C2) {
    A4 ? g2(A4) : I2(C2.buffer);
  });
}, process.argv.length > 1 && process.argv[1].replace(/\\/g, "/"), D = process.argv.slice(2), "undefined" != typeof module && (module.exports = E), process.on("uncaughtException", function(A3) {
  if (!(A3 instanceof BA)) throw A3;
}), process.on("unhandledRejection", function(A3) {
  throw A3;
}), o = (A3, I2) => {
  if (d()) throw process.exitCode = A3, I2;
  !function(A4) {
    if (A4 instanceof BA) return;
    N("exiting due to exception: " + A4);
  }(I2), process.exit(A3);
}, E.inspect = function() {
  return "[Emscripten Module object]";
}) : (y || h) && (h ? a = self.location.href : "undefined" != typeof document && document.currentScript && (a = document.currentScript.src), a = 0 !== a.indexOf("blob:") ? a.substr(0, a.replace(/[?#].*/, "").lastIndexOf("/") + 1) : "", A = (A3) => {
  try {
    var I2 = new XMLHttpRequest();
    return I2.open("GET", A3, false), I2.send(null), I2.responseText;
  } catch (I3) {
    var g2 = _(A3);
    if (g2) return function(A4) {
      for (var I4 = [], g3 = 0; g3 < A4.length; g3++) {
        var C2 = A4[g3];
        C2 > 255 && (C2 &= 255), I4.push(String.fromCharCode(C2));
      }
      return I4.join("");
    }(g2);
    throw I3;
  }
}, h && (g = (A3) => {
  try {
    var I2 = new XMLHttpRequest();
    return I2.open("GET", A3, false), I2.responseType = "arraybuffer", I2.send(null), new Uint8Array(I2.response);
  } catch (I3) {
    var g2 = _(A3);
    if (g2) return g2;
    throw I3;
  }
}), I = (A3, I2, g2) => {
  var C2 = new XMLHttpRequest();
  C2.open("GET", A3, true), C2.responseType = "arraybuffer", C2.onload = () => {
    if (200 == C2.status || 0 == C2.status && C2.response) I2(C2.response);
    else {
      var Q2 = _(A3);
      Q2 ? I2(Q2.buffer) : g2();
    }
  }, C2.onerror = g2, C2.send(null);
});
var s;
var w = E.print || console.log.bind(console);
var N = E.printErr || console.warn.bind(console);
Object.assign(E, i), i = null, E.arguments && (D = E.arguments), E.thisProgram && E.thisProgram, E.quit && (o = E.quit), E.wasmBinary && (s = E.wasmBinary);
var F;
var R = E.noExitRuntime || true;
"object" != typeof WebAssembly && m("no native wasm support detected");
var t = false;
function S(A3) {
  return E["_" + A3];
}
function M(A3, I2, g2, C2, Q2) {
  var B2 = { string: function(A4) {
    var I3 = 0;
    if (null != A4 && 0 !== A4) {
      var g3 = 1 + (A4.length << 2);
      (function(A5, I4, g4) {
        (function(A6, I5, g5, C3) {
          if (!(C3 > 0)) return 0;
          for (var Q3 = g5, B3 = g5 + C3 - 1, E3 = 0; E3 < A6.length; ++E3) {
            var i3 = A6.charCodeAt(E3);
            if (i3 >= 55296 && i3 <= 57343) i3 = 65536 + ((1023 & i3) << 10) | 1023 & A6.charCodeAt(++E3);
            if (i3 <= 127) {
              if (g5 >= B3) break;
              I5[g5++] = i3;
            } else if (i3 <= 2047) {
              if (g5 + 1 >= B3) break;
              I5[g5++] = 192 | i3 >> 6, I5[g5++] = 128 | 63 & i3;
            } else if (i3 <= 65535) {
              if (g5 + 2 >= B3) break;
              I5[g5++] = 224 | i3 >> 12, I5[g5++] = 128 | i3 >> 6 & 63, I5[g5++] = 128 | 63 & i3;
            } else {
              if (g5 + 3 >= B3) break;
              I5[g5++] = 240 | i3 >> 18, I5[g5++] = 128 | i3 >> 12 & 63, I5[g5++] = 128 | i3 >> 6 & 63, I5[g5++] = 128 | 63 & i3;
            }
          }
          I5[g5] = 0;
        })(A5, k, I4, g4);
      })(A4, I3 = QA(g3), g3);
    }
    return I3;
  }, array: function(A4) {
    var I3 = QA(A4.length);
    return function(A5, I4) {
      r.set(A5, I4);
    }(A4, I3), I3;
  } };
  var E2 = S(A3), i2 = [], D2 = 0;
  if (C2) for (var o2 = 0; o2 < C2.length; o2++) {
    var y2 = B2[g2[o2]];
    y2 ? (0 === D2 && (D2 = gA()), i2[o2] = y2(C2[o2])) : i2[o2] = C2[o2];
  }
  var h2 = E2.apply(null, i2);
  return h2 = function(A4) {
    return 0 !== D2 && CA(D2), function(A5) {
      return "string" === I2 ? Y(A5) : "boolean" === I2 ? Boolean(A5) : A5;
    }(A4);
  }(h2);
}
var c;
var r;
var k;
var n;
var K = "undefined" != typeof TextDecoder ? new TextDecoder("utf8") : void 0;
function J(A3, I2, g2) {
  for (var C2 = I2 + g2, Q2 = I2; A3[Q2] && !(Q2 >= C2); ) ++Q2;
  if (Q2 - I2 > 16 && A3.subarray && K) return K.decode(A3.subarray(I2, Q2));
  for (var B2 = ""; I2 < Q2; ) {
    var E2 = A3[I2++];
    if (128 & E2) {
      var i2 = 63 & A3[I2++];
      if (192 != (224 & E2)) {
        var D2 = 63 & A3[I2++];
        if ((E2 = 224 == (240 & E2) ? (15 & E2) << 12 | i2 << 6 | D2 : (7 & E2) << 18 | i2 << 12 | D2 << 6 | 63 & A3[I2++]) < 65536) B2 += String.fromCharCode(E2);
        else {
          var o2 = E2 - 65536;
          B2 += String.fromCharCode(55296 | o2 >> 10, 56320 | 1023 & o2);
        }
      } else B2 += String.fromCharCode((31 & E2) << 6 | i2);
    } else B2 += String.fromCharCode(E2);
  }
  return B2;
}
function Y(A3, I2) {
  return A3 ? J(k, A3, I2) : "";
}
function U(A3) {
  c = A3, E.HEAP8 = r = new Int8Array(A3), E.HEAP16 = new Int16Array(A3), E.HEAP32 = n = new Int32Array(A3), E.HEAPU8 = k = new Uint8Array(A3), E.HEAPU16 = new Uint16Array(A3), E.HEAPU32 = new Uint32Array(A3), E.HEAPF32 = new Float32Array(A3), E.HEAPF64 = new Float64Array(A3);
}
E.INITIAL_MEMORY;
var e;
var L = [];
var H = [];
var f = [];
var l = false;
function d() {
  return R || false;
}
var q = 0;
var b = null;
var u = null;
function m(A3) {
  throw E.onAbort && E.onAbort(A3), N(A3 = "Aborted(" + A3 + ")"), t = true, 1, A3 += ". Build with -s ASSERTIONS=1 for more info.", new WebAssembly.RuntimeError(A3);
}
E.preloadedImages = {}, E.preloadedAudios = {};
var Z;
var x;
function p(A3) {
  return A3.startsWith("data:application/octet-stream;base64,");
}
function W(A3) {
  return A3.startsWith("file://");
}
function T(A3) {
  try {
    if (A3 == Z && s) return new Uint8Array(s);
    var I2 = _(A3);
    if (I2) return I2;
    if (g) return g(A3);
    throw "both async and sync fetching of the wasm failed";
  } catch (A4) {
    m(A4);
  }
}
function z(A3) {
  for (; A3.length > 0; ) {
    var I2 = A3.shift();
    if ("function" != typeof I2) {
      var g2 = I2.func;
      "number" == typeof g2 ? void 0 === I2.arg ? X(g2)() : X(g2)(I2.arg) : g2(void 0 === I2.arg ? null : I2.arg);
    } else I2(E);
  }
}
p(Z = "data:application/octet-stream;base64,") || (x = Z, Z = E.locateFile ? E.locateFile(x, a) : a + x);
var j = [];
function X(A3) {
  var I2 = j[A3];
  return I2 || (A3 >= j.length && (j.length = A3 + 1), j[A3] = I2 = e.get(A3)), I2;
}
function O(A3) {
  this.excPtr = A3, this.ptr = A3 - 16, this.set_type = function(A4) {
    n[this.ptr + 4 >> 2] = A4;
  }, this.get_type = function() {
    return n[this.ptr + 4 >> 2];
  }, this.set_destructor = function(A4) {
    n[this.ptr + 8 >> 2] = A4;
  }, this.get_destructor = function() {
    return n[this.ptr + 8 >> 2];
  }, this.set_refcount = function(A4) {
    n[this.ptr >> 2] = A4;
  }, this.set_caught = function(A4) {
    A4 = A4 ? 1 : 0, r[this.ptr + 12 >> 0] = A4;
  }, this.get_caught = function() {
    return 0 != r[this.ptr + 12 >> 0];
  }, this.set_rethrown = function(A4) {
    A4 = A4 ? 1 : 0, r[this.ptr + 13 >> 0] = A4;
  }, this.get_rethrown = function() {
    return 0 != r[this.ptr + 13 >> 0];
  }, this.init = function(A4, I2) {
    this.set_type(A4), this.set_destructor(I2), this.set_refcount(0), this.set_caught(false), this.set_rethrown(false);
  }, this.add_ref = function() {
    var A4 = n[this.ptr >> 2];
    n[this.ptr >> 2] = A4 + 1;
  }, this.release_ref = function() {
    var A4 = n[this.ptr >> 2];
    return n[this.ptr >> 2] = A4 - 1, 1 === A4;
  };
}
function V(A3) {
  try {
    return F.grow(A3 - c.byteLength + 65535 >>> 16), U(F.buffer), 1;
  } catch (A4) {
  }
}
var v = { buffers: [null, [], []], printChar: function(A3, I2) {
  var g2 = v.buffers[A3];
  0 === I2 || 10 === I2 ? ((1 === A3 ? w : N)(J(g2, 0)), g2.length = 0) : g2.push(I2);
}, varargs: void 0, get: function() {
  return v.varargs += 4, n[v.varargs - 4 >> 2];
}, getStr: function(A3) {
  return Y(A3);
}, get64: function(A3, I2) {
  return A3;
} };
var P = "function" == typeof atob ? atob : function(A3) {
  var I2, g2, C2, Q2, B2, E2, i2 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=", D2 = "", o2 = 0;
  A3 = A3.replace(/[^A-Za-z0-9\+\/\=]/g, "");
  do {
    I2 = i2.indexOf(A3.charAt(o2++)) << 2 | (Q2 = i2.indexOf(A3.charAt(o2++))) >> 4, g2 = (15 & Q2) << 4 | (B2 = i2.indexOf(A3.charAt(o2++))) >> 2, C2 = (3 & B2) << 6 | (E2 = i2.indexOf(A3.charAt(o2++))), D2 += String.fromCharCode(I2), 64 !== B2 && (D2 += String.fromCharCode(g2)), 64 !== E2 && (D2 += String.fromCharCode(C2));
  } while (o2 < A3.length);
  return D2;
};
function _(A3) {
  if (p(A3)) return function(A4) {
    if ("boolean" == typeof G && G) {
      var I2 = Buffer.from(A4, "base64");
      return new Uint8Array(I2.buffer, I2.byteOffset, I2.byteLength);
    }
    try {
      for (var g2 = P(A4), C2 = new Uint8Array(g2.length), Q2 = 0; Q2 < g2.length; ++Q2) C2[Q2] = g2.charCodeAt(Q2);
      return C2;
    } catch (A5) {
      throw new Error("Converting base64 string to bytes failed.");
    }
  }(A3.slice("data:application/octet-stream;base64,".length));
}
var $;
var AA = { c: function(A3) {
  return IA(A3 + 16) + 16;
}, b: function(A3, I2, g2) {
  throw new O(A3).init(I2, g2), A3, A3;
}, f: function() {
  m("");
}, i: function(A3, I2, g2) {
  k.copyWithin(A3, I2, I2 + g2);
}, g: function(A3) {
  var I2, g2, C2 = k.length, Q2 = 2147483648;
  if ((A3 >>>= 0) > Q2) return false;
  for (var B2 = 1; B2 <= 4; B2 *= 2) {
    var E2 = C2 * (1 + 0.2 / B2);
    if (E2 = Math.min(E2, A3 + 100663296), V(Math.min(Q2, (I2 = Math.max(A3, E2)) + ((g2 = 65536) - I2 % g2) % g2))) return true;
  }
  return false;
}, d: function(A3) {
  !function(A4, I2) {
    A4, d() || true;
    !function(A5) {
      A5, d() || (E.onExit && E.onExit(A5), t = true);
      o(A5, new BA(A5));
    }(A4);
  }(A3);
}, h: function(A3) {
  return 0;
}, e: function(A3, I2, g2, C2, Q2) {
}, a: function(A3, I2, g2, C2) {
  for (var Q2 = 0, B2 = 0; B2 < g2; B2++) {
    var E2 = n[I2 >> 2], i2 = n[I2 + 4 >> 2];
    I2 += 8;
    for (var D2 = 0; D2 < i2; D2++) v.printChar(A3, k[E2 + D2]);
    Q2 += i2;
  }
  return n[C2 >> 2] = Q2, 0;
} };
var IA = (function() {
  var A3 = { a: AA };
  function g2(A4, I2) {
    var g3, C3 = A4.exports;
    E.asm = C3, U((F = E.asm.j).buffer), e = E.asm.n, g3 = E.asm.k, H.unshift(g3), function(A5) {
      if (q--, E.monitorRunDependencies && E.monitorRunDependencies(q), 0 == q && (null !== b && (clearInterval(b), b = null), u)) {
        var I3 = u;
        u = null, I3();
      }
    }();
  }
  function C2(A4) {
    g2(A4.instance);
  }
  function Q2(g3) {
    return function() {
      if (!s && (y || h)) {
        if ("function" == typeof fetch && !W(Z)) return fetch(Z, { credentials: "same-origin" }).then(function(A4) {
          if (!A4.ok) throw "failed to load wasm binary file at '" + Z + "'";
          return A4.arrayBuffer();
        }).catch(function() {
          return T(Z);
        });
        if (I) return new Promise(function(A4, g4) {
          I(Z, function(I2) {
            A4(new Uint8Array(I2));
          }, g4);
        });
      }
      return Promise.resolve().then(function() {
        return T(Z);
      });
    }().then(function(I2) {
      return WebAssembly.instantiate(I2, A3);
    }).then(function(A4) {
      return A4;
    }).then(g3, function(A4) {
      N("failed to asynchronously prepare wasm: " + A4), m(A4);
    });
  }
  if (q++, E.monitorRunDependencies && E.monitorRunDependencies(q), E.instantiateWasm) try {
    return E.instantiateWasm(A3, g2);
  } catch (A4) {
    return N("Module.instantiateWasm callback failed with error: " + A4), false;
  }
  s || "function" != typeof WebAssembly.instantiateStreaming || p(Z) || W(Z) || "function" != typeof fetch ? Q2(C2) : fetch(Z, { credentials: "same-origin" }).then(function(I2) {
    return WebAssembly.instantiateStreaming(I2, A3).then(C2, function(A4) {
      return N("wasm streaming compile failed: " + A4), N("falling back to ArrayBuffer instantiation"), Q2(C2);
    });
  });
}(), E.___wasm_call_ctors = function() {
  return (E.___wasm_call_ctors = E.asm.k).apply(null, arguments);
}, E._malloc = function() {
  return (IA = E._malloc = E.asm.l).apply(null, arguments);
});
var gA = (E._start = function() {
  return (E._start = E.asm.m).apply(null, arguments);
}, E.stackSave = function() {
  return (gA = E.stackSave = E.asm.o).apply(null, arguments);
});
var CA = E.stackRestore = function() {
  return (CA = E.stackRestore = E.asm.p).apply(null, arguments);
};
var QA = E.stackAlloc = function() {
  return (QA = E.stackAlloc = E.asm.q).apply(null, arguments);
};
function BA(A3) {
  this.name = "ExitStatus", this.message = "Program terminated with exit(" + A3 + ")", this.status = A3;
}
function EA(A3) {
  function I2() {
    $ || ($ = true, E.calledRun = true, t || (l = true, z(H), E.onRuntimeInitialized && E.onRuntimeInitialized(), function() {
      if (E.postRun) for ("function" == typeof E.postRun && (E.postRun = [E.postRun]); E.postRun.length; ) A4 = E.postRun.shift(), f.unshift(A4);
      var A4;
      z(f);
    }()));
  }
  A3 = A3 || D, q > 0 || (!function() {
    if (E.preRun) for ("function" == typeof E.preRun && (E.preRun = [E.preRun]); E.preRun.length; ) A4 = E.preRun.shift(), L.unshift(A4);
    var A4;
    z(L);
  }(), q > 0 || (E.setStatus ? (E.setStatus("Running..."), setTimeout(function() {
    setTimeout(function() {
      E.setStatus("");
    }, 1), I2();
  }, 1)) : I2()));
}
if (u = function A2() {
  $ || EA(), $ || (u = A2);
}, E.run = EA, E.preInit) for ("function" == typeof E.preInit && (E.preInit = [E.preInit]); E.preInit.length > 0; ) E.preInit.pop()();
EA();
var iA = async (A3, I2 = {}) => {
  let g2;
  I2 = Object.assign({ turdsize: 2, turnpolicy: 4, alphamax: 1, opticurve: 1, opttolerance: 0.2, pathonly: false, extractcolors: true, posterizelevel: 5, posterizationalgorithm: 1 }, I2);
  const C2 = A3.constructor.name;
  if ("Blob" === C2) g2 = await (async () => new Promise((I3) => {
    const g3 = URL.createObjectURL(A3), C3 = new Image();
    C3.onload = () => {
      const A4 = document.createElement("canvas"), g4 = A4.getContext("2d");
      if (!g4) throw new Error("Canvas is not supported.");
      const Q3 = C3;
      A4.width = Number(Q3.width), A4.height = Number(Q3.height), g4.drawImage(Q3, 0, 0, A4.width, A4.height), I3(g4.getImageData(0, 0, A4.width, A4.height));
    }, C3.src = g3;
  }))();
  else if ("HTMLImageElement" === C2 || "SVGImageElement" === C2 || "HTMLVideoElement" === C2 || "HTMLCanvasElement" === C2 || "ImageBitmap" === C2) {
    const I3 = document.createElement("canvas"), C3 = I3.getContext("2d");
    if (!C3) throw new Error("Canvas is not supported.");
    const Q3 = A3;
    I3.width = Number(Q3.width), I3.height = Number(Q3.height), C3.drawImage(Q3, 0, 0, I3.width, I3.height), g2 = C3.getImageData(0, 0, I3.width, I3.height);
  } else g2 = A3;
  const Q2 = (B2 = "start", E2 = "string", o2 = (i2 = (i2 = ["array", "number", "number", "number", "number", "number", "number", "number", "number", "number", "number", "number", "number"]) || []).every(function(A4) {
    return "number" === A4;
  }), "string" !== E2 && o2 && !D2 ? S(B2) : function() {
    return M(B2, E2, i2, arguments);
  });
  var B2, E2, i2, D2, o2;
  await DA();
  const y2 = Q2(g2.data, g2.width, g2.height, true, I2.pathonly, I2.extractcolors, I2.posterizelevel, I2.posterizationalgorithm, I2.turdsize, I2.turnpolicy, I2.alphamax, I2.opticurve, I2.opttolerance);
  return I2.pathonly ? y2.split("M").filter((A4) => A4).map((A4) => "M" + A4) : y2;
};
function DA() {
  return new Promise((A3) => {
    l ? A3() : E.onRuntimeInitialized = () => {
      A3();
    };
  });
}
if ("undefined" != typeof module) {
  const A3 = DA;
  module.exports = { potrace: iA, init: A3 };
}
export {
  DA as init,
  iA as potrace
};
//# sourceMappingURL=esm-potrace-wasm.js.map
